...
A regex is also sometimes called a "pattern" because we're looking for a
portion of a string that looks like the strings that the regex
describes. But what string are we looking in? As in Perl 5, Perl 6
applies the regex to a variable called C<$_> if we haven't explicitly
specified a variable to match against. For now I'm going to continue
assuming that our string is in C<$_> in my examples. Later I'll show you
how to specify a different string to match against.
...
Now, if all you can do is match literal strings, regexes wouldn't be very
useful, would they? Some characters, rather than taken literally, are 
so-called metacharacters that have special meaning in a regex. In Perl 6,
...
metacharacter and has special meaning. By the same token, prefixing a 
non-alphanumeric character with a backslash removes its metasyntactic 
nature and it becomes literal.
...
To match a literal period character you can escape it like so:
...
Some very important metasyntactic characters in regex are whitespace
...
space characters or other whitespace you should enclose them in quotes.

=head2 Quantifiers

Three other important metacharacters are known as quantifiers.
...
two C<*> characters followed by a number or a range like so:
    ...
    / f o**3 /     # matches only "fooo"
    / f o**3..5 /  # matches one of "fooo", "foooo", or "fooooo"

You may also specify a closure after the C<**>, but that is beyond the
scope of this introduction to explain.  For more information on quantifiers
see L<the Quantifiers section|https://docs.perl6.org/language/regexes#Quantifiers>
of L<the Perl 6 documentation|https://docs.perl6.org/>.
...
and "subtract" character classes much like you would sets:
    ...
    /<[asdfg] + [hjkl;]> +/ # Only match a sequence of characters that can
                            # be made from home row keys of many qwerty layouts
...
Jonathan Scott Duff is a senior software developer at Assurant, Inc.
He has a beautiful wife and 4 lovely children. When not working or 
spending time with his family, Scott tries to keep up with Perl 6 
development and do a little wood working or spend time in nature.
Sometimes he can be found on IRC as either perlpilot or PerlJam in
one of the perl-related channels.  If you want to get in touch with 
him, the best way is via email: duff@pobox.com


Copyright 2007-2018 Jonathan Scott Duff
